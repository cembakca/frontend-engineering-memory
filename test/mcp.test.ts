...
test("exposes bounded read-only memory tools over MCP",async()=>{
  ...
  try {
    ...
    memoryDb.db.prepare("INSERT INTO repositories(name,path,next_version,router_type,last_indexed_sha) VALUES(?,?,?,?,?)").run("second",root,"16.0.0","app","b".repeat(40));
    ...
    const located=await client.callTool({name:"memory_route",arguments:{route:"/hakkımızda"}});
    assert.deepEqual((located.structuredContent as any).matches.map((item:any)=>item.repository),["fixture","second"]);
    assert.equal((located.structuredContent as any).normalizedRoute,"/hakkimizda");
